...
export const HeroPage = () => {
  const {idSlug = ""} = useParams(); // Replace with useParams() in real scenario
  const {data: superheroData, isError} = useHero(idSlug);

  if (isError) {
    return <div className="flex items-center justify-center min-h-screen">Error al cargar el superhéroe.</div>
  }
